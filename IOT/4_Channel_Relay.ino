...
void setup()
{
    ...
    // Relay is active LOW — HIGH means relay OFF (safe default)
...
void handleDeviceControl(String device, String action)
{
    ...
    if (action == "on")
    {
        digitalWrite(pin, LOW); // Relay active LOW — LOW energizes the relay
        *statePtr = true;
    }
    else if (action == "off")
    {
        digitalWrite(pin, HIGH); // HIGH de-energizes the relay
        *statePtr = false;
    }
    else if (action == "toggle")
    {
        *statePtr = !(*statePtr);
        digitalWrite(pin, *statePtr ? LOW : HIGH); // LOW = ON, HIGH = OFF
    }
