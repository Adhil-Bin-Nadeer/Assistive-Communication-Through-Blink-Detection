...
document.addEventListener('DOMContentLoaded', () => {
    ...
    else if (document.body.classList.contains('message-page')) {
        // Morse input page buttons
        document.getElementById('speakBtn')?.addEventListener('click', () => {
            state.socket.emit('speak_message');
        });
        document.getElementById('clearBtn')?.addEventListener('click', () => {
            state.socket.emit('clear_message');
        });
        document.getElementById('deleteLastBtn')?.addEventListener('click', () => {
            state.socket.emit('delete_last_char');
        });
    }
    ...
    if (document.body.classList.contains('device-control-page')) {
        ...
        if (deviceTitle) {
            const names = { light1: 'Light 1', light2: 'Light 2', fan: 'Fan', ac: 'AC' };
            deviceTitle.textContent = names[device] || device;
        }
    ...
    const streamStartDelay = isMessagePage ? 3500 : 500;

    // Run countdown on message page
    if (isMessagePage) {
    ...
    setTimeout(() => {
        if (document.body.classList.contains('device-control-page')) {
            ...
            if (!state.isStreaming || !state.stream || !state.stream.active) {
                state.socket.emit('select_user', { username: state.currentSelectedUser }, (response) => {
                    if (response.status === 'success') {
                        // Start webcam early on message page for preview
                        initWebcam((err) => {
                            if (err) {
                                updateStatus('Webcam access failed. Please allow camera access and reload the page.');
                            }
